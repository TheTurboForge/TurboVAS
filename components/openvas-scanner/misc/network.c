      ...
      for (t = 0; total < max_len && (timeout <= 0 || t < timeout);)
        {
          ...
          if (select (realfd + 1, &fdr, NULL, NULL, timeout > 0 ? &tv : NULL)
            ...
            {
              errno = 0;
              // Generic NASL socket receive primitive; protocol encryption is
              // script-specific and cannot be forced at this transport layer.
...
write_stream_connection4 (int fd, void *buf0, int n, int i_opt)
{
  ...
  switch (fp->transport)
    ...
    case OPENVAS_ENCAPS_IP:
      for (count = 0; count < n;)
        {
          // Generic NASL socket send primitive; protocol encryption is
          // script-specific and cannot be forced at this transport layer.
          // codeql[cpp/cleartext-transmission]
