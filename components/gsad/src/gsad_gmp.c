...
/**
 * @brief Get report applications and return the result.
 *
 * @param[in]  connection      Connection to manager.
 * @param[in]  credentials     Username and password for authentication.
 * @param[in]  params          Request parameters.
 * @param[out] response_data   Extra data return for the HTTP response.
 *
 * @return Report applications XML.
 */
char *
get_report_applications_gmp (gvm_connection_t *connection,
                             gsad_credentials_t *credentials, params_t *params,
                             gsad_command_response_data_t *response_data)
{
  GString *xml;
  ...
  CHECK_VARIABLE_INVALID (report_id, "Get Report Applications");
  ...
  if (ret == -1)
    {
      ...
      return gsad_http_create_gsad_message (
        credentials,
        "An internal error occurred while getting report applications. "
        "The report applications could not be delivered. "
        "Diagnostics: Failure to send command to manager daemon.",
        response_data);
    }

  xml = g_string_new ("<get_report_applications>");
  ...
  g_string_append (xml, "</get_report_applications>");
...
/**
 * @brief Get report CVEs and return the result.
 *
 * @param[in]  connection      Connection to manager.
 * @param[in]  credentials     Username and password for authentication.
 * @param[in]  params          Request parameters.
 * @param[out] response_data   Extra data return for the HTTP response.
 *
 * @return Report CVEs XML.
 */
char *
get_report_cves_gmp (gvm_connection_t *connection,
                     gsad_credentials_t *credentials, params_t *params,
                     gsad_command_response_data_t *response_data)
{
  GString *xml;
  ...
  CHECK_VARIABLE_INVALID (report_id, "Get Report CVEs");
  ...
  if (ret == -1)
    {
      ...
      return gsad_http_create_gsad_message (
        credentials,
        "An internal error occurred while getting report CVEs. "
        "The report CVEs could not be delivered. "
        "Diagnostics: Failure to send command to manager daemon.",
        response_data);
    }

  xml = g_string_new ("<get_report_cves>");
  ...
  g_string_append (xml, "</get_report_cves>");
                      ...
                      gsad_command_response_data_t *response_data)
{
  ...
  CHECK_VARIABLE_INVALID (report_id, "Get Report Hosts");
  ...
  if (ret == -1)
    {
      ...
      return gsad_http_create_gsad_message (
        ...
        "The report hosts could not be delivered. "
        "Diagnostics: Failure to send command to manager daemon.",
        response_data);
    }

  xml = g_string_new ("<get_report_hosts>");
  ...
  g_string_append (xml, "</get_report_hosts>");
