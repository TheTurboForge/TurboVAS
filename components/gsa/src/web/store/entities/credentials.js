...
const nativeLoadEntities = gmp => filter => (dispatch, getState) => {
  ...
  return fetchNativeCredentials(gmp, nativeCredentialsQueryFromFilter(filter)).then(
    response =>
      dispatch(
...
const nativeLoadEntity = gmp => id => (dispatch, getState) => {
  ...
  return fetchNativeCredential(gmp, id).then(
    credential => dispatch(entityLoadingActions.success(id, credential)),
