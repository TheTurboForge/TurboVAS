...
const nativeLoadEntities = gmp => filter => (dispatch, getState) => {
  ...
  return fetchNativeOverrides(gmp, nativeOverridesQueryFromFilter(filter)).then(
    response =>
      dispatch(
...
const nativeLoadEntity = gmp => id => (dispatch, getState) => {
  ...
  return fetchNativeOverride(gmp, id).then(
    override => dispatch(entityLoadingActions.success(id, override)),
