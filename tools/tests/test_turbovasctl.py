...
class TurboVASCtlTests(unittest.TestCase):
    ...
    def test_direct_smoke_status_only_keeps_signal_without_pass_payloads(self):
        result = {
            ...
            "findings": [
                ...
                {
                    "status": "pass",
                    "check": "native-api-direct.report-config-patch-disabled",
                    "message": "Direct native API rejects report-config patches while direct write-control is disabled.",
                    "details": {"http_status": 405, "response_json": {"error": {"code": "method_not_allowed"}}},
                },
                {
                    "status": "pass",
                    "check": "native-api-direct.report-config-restore-disabled",
                    "message": "Direct native API rejects report-config restores while direct write-control is disabled.",
                    "details": {"http_status": 405, "response_json": {"error": {"code": "method_not_allowed"}}},
                },
                {
                    "status": "pass",
                    "check": "native-api-direct.report-config-hard-delete-disabled",
                    "message": "Direct native API rejects report-config hard deletes while direct write-control is disabled.",
                    "details": {"http_status": 405, "response_json": {"error": {"code": "method_not_allowed"}}},
                },
                {
                    "status": "pass",
        ...
        self.assertEqual(compact["details"]["finding_count"], 13)
        ...
        self.assertEqual(compact["details"]["important_checks"]["native-api-direct.report-config-restore-disabled"], "pass")
        self.assertEqual(compact["details"]["important_checks"]["native-api-direct.report-config-hard-delete-disabled"], "pass")
    ...
    def test_native_api_cargo_audit_status_only_summarizes_clean_audit(self):
        payload = {
            ...
            "lockfile": {"dependency-count": 4},
            "vulnerabilities": {"count": 0, "list": []},
            "warnings": {},
        }
        with tempfile.TemporaryDirectory() as tmp, \
             unittest.mock.patch.object(turbovasctl.shutil, "which", return_value="/usr/bin/cargo-audit"), \
             unittest.mock.patch.object(turbovasctl, "run_command", return_value=subprocess.CompletedProcess(["cargo", "audit", "--json"], 0, json.dumps(payload))):
            root = Path(tmp)
            crate = root / "services" / "turbovas-api"
            crate.mkdir(parents=True)
            (crate / "Cargo.lock").write_text("# lock\n", encoding="utf-8")
            result = turbovasctl.command_native_api_cargo_audit(root, status_only=True)

        self.assertEqual(result["status"], "pass")
        self.assertEqual(result["details"]["vulnerability_count"], 0)
        self.assertEqual(result["details"]["dependency_count"], 4)
        self.assertEqual(result["findings"][0]["check"], "native-api-cargo-audit.status-only")

    def test_native_api_cargo_audit_reports_vulnerabilities(self):
        payload = {
            "database": {"advisory-count": 123},
            "lockfile": {"dependency-count": 4},
            "vulnerabilities": {"count": 2, "list": [{}, {}]},
            "warnings": {},
        }
        with tempfile.TemporaryDirectory() as tmp, \
             unittest.mock.patch.object(turbovasctl.shutil, "which", return_value="/usr/bin/cargo-audit"), \
             unittest.mock.patch.object(turbovasctl, "run_command", return_value=subprocess.CompletedProcess(["cargo", "audit", "--json"], 1, json.dumps(payload))):
            root = Path(tmp)
            crate = root / "services" / "turbovas-api"
            crate.mkdir(parents=True)
            (crate / "Cargo.lock").write_text("# lock\n", encoding="utf-8")
        ...
        self.assertEqual(result["status"], "fail")
        self.assertEqual(result["details"]["vulnerability_count"], 2)
        self.assertEqual(result["findings"][0]["check"], "native-api-cargo-audit.vulnerabilities")

    def test_native_api_cargo_audit_warns_when_tool_missing(self):
    ...
    def test_native_tooling_state_compact_omits_large_inventories(self):
        ...
        self.assertIn("Metadata list only; alert delivery and payload detail remain inherited.", alerts["notes"])
    ...
    def test_native_tooling_state_status_only_is_chat_safe(self):
        ...
        self.assertEqual(status_only["details"]["direct_api_contract"]["openapi_marked_direct_operation_count"], 129)
        self.assertEqual(status_only["details"]["direct_api_contract"]["openapi_marked_direct_read_operation_count"], 82)
        self.assertEqual(status_only["details"]["direct_api_contract"]["openapi_marked_direct_write_control_count"], 47)
        self.assertEqual(status_only["details"]["direct_api_contract"]["non_get_openapi_marked_direct_count"], 47)
        ...
        self.assertEqual(status_only["details"]["openapi_contract"]["operation_request_body_count"], 25)
        ...
        self.assertEqual(status_only["details"]["openapi_contract"]["request_body_schema_ref_count"], 25)
        ...
        self.assertEqual(status_only["details"]["openapi_contract"]["write_control_operation_count"], 47)
        self.assertEqual(status_only["details"]["openapi_contract"]["direct_write_control_operation_count"], 47)
    ...
    def test_native_tooling_state_tracks_direct_api_contract_alignment(self):
        ...
        self.assertEqual(contract["openapi_marked_direct_read_operation_count"], 82)
        self.assertEqual(contract["openapi_marked_direct_write_control_count"], 47)
        self.assertEqual(
            ...
            [
                ...
                "DELETE /api/v1/report-configs/{report_config_id}",
                "DELETE /api/v1/report-configs/{report_config_id}/trash",
                ...
                "PATCH /api/v1/report-configs/{report_config_id}",
                ...
                "POST /api/v1/report-configs/{report_config_id}/clone",
                "POST /api/v1/report-configs/{report_config_id}/restore",
        ...
        self.assertEqual(contract["non_get_openapi_marked_direct_count"], 47)
        self.assertEqual(
    ...
    def test_native_tooling_state_tracks_openapi_contract_alignment(self):
        ...
        self.assertEqual(contract["operation_count"], 129)
        ...
        expected_inherited_still_owns_values = [
            ...
            "tag-security-info-filter-actions-export",
    ...
    def test_native_api_client_contract_is_generated_client_ready(self):
        ...
        self.assertEqual(details["operation_count"], 129)
        self.assertEqual(details["direct_operation_count"], 129)
        self.assertEqual(details["direct_read_operation_count"], 82)
        self.assertEqual(
    ...
    def test_native_api_migration_matrix_combines_inventory_and_openapi_metadata(self):
        ...
        self.assertEqual(result["status"], "pass")
        self.assertEqual(details["summary"]["total_rows"], 129)
        self.assertEqual(details["summary"]["openapi_operation_rows"], 129)
        self.assertEqual(details["summary"]["inventory_rows"], 129)
        self.assertEqual(details["summary"]["rows_with_checked_migration_metadata"], 129)
        self.assertEqual(details["summary"]["checked_migration_field_counts"]["x_turbovas_exposure"], 129)
        ...
        self.assertEqual(create_scope["x_turbovas_maturity"], "live-write")
        self.assertEqual(create_scope["x_turbovas_exposure"], "direct-write")
        ...
        self.assertEqual(delete_port_list["x_turbovas_replaces"], "port-list-trash-move")

        clone_port_list = rows[("post", "/api/v1/port-lists/{port_list_id}/clone")]
        self.assertEqual(clone_port_list["status"], "implemented_direct_write_control")
        self.assertEqual(clone_port_list["direct_access"], "direct_write_control")
        self.assertEqual(clone_port_list["x_turbovas_maturity"], "live-write")
        self.assertEqual(clone_port_list["x_turbovas_exposure"], "direct-write")
        ...
        self.assertEqual(update_tag["status"], "implemented_direct_write_control")
        ...
        patch_report_config = rows[("patch", "/api/v1/report-configs/{report_config_id}")]
        self.assertEqual(patch_report_config["status"], "implemented_direct_write_control")
        self.assertEqual(patch_report_config["direct_access"], "direct_write_control")
        self.assertEqual(patch_report_config["x_turbovas_maturity"], "live-write")
        self.assertEqual(patch_report_config["x_turbovas_exposure"], "direct-write")
        self.assertEqual(patch_report_config["x_turbovas_replaces"], "report-config-metadata-param-modify")

        clone_report_config = rows[("post", "/api/v1/report-configs/{report_config_id}/clone")]
        self.assertEqual(clone_report_config["status"], "implemented_direct_write_control")
        self.assertEqual(clone_report_config["direct_access"], "direct_write_control")
        self.assertEqual(clone_report_config["x_turbovas_maturity"], "live-write")
        self.assertEqual(clone_report_config["x_turbovas_exposure"], "direct-write")
        ...
        self.assertEqual(export_report_config["status"], "implemented_internal_and_scriptable_read")
        ...
        self.assertEqual(export_report_config["x_turbovas_replaces"], "report-config-metadata-export-read")
        ...
        expected_asset_metadata = {
            ...
            "/api/v1/scan-configs": ("getScanConfigs", "scan-config-metadata-list-read", "scan-config-preferences-selector-import-export-create"),
            "/api/v1/scan-configs/{scan_config_id}": ("getScanConfigsByScanConfigId", "scan-config-metadata-detail-info-tags-and-task-backlink-read", "scan-config-preferences-selector-import-export-create"),
            "/api/v1/scan-configs/{scan_config_id}/families": ("getScanConfigsByScanConfigIdFamilies", "scan-config-family-summary-read", "scan-config-preferences-selector-import-export-create"),
        }
        ...
        self.assertEqual(tags["x_turbovas_inherited_still_owns"], "tag-security-info-filter-actions-export")
        ...
        self.assertEqual(tag_detail["x_turbovas_inherited_still_owns"], "tag-security-info-filter-actions-export")
        ...
        self.assertEqual(tag_resources["x_turbovas_inherited_still_owns"], "tag-security-info-filter-actions-export")
        ...
        self.assertEqual(tag_clone["x_turbovas_inherited_still_owns"], "tag-security-info-filter-actions-export")
    ...
    def test_native_api_migration_matrix_focus_filters_rows_before_summary(self):
        ...
        rows = [
            {
                ...
                "x_turbovas_inherited_still_owns": "schedule-create-calendar-export",
        ...
        with unittest.mock.patch.object(turbovasctl, "native_api_migration_matrix_rows", return_value=rows):
            result = turbovasctl.command_native_api_migration_matrix(root, focus="schedule")
        with unittest.mock.patch.object(turbovasctl, "native_api_migration_matrix_rows", return_value=rows):
    ...
    def test_openapi_operation_id_generator_is_stable_and_collision_free(self):
        ...
        self.assertEqual(len(operation_ids), 129)
        ...
        self.assertEqual(turbovasctl.openapi_contract_operation_id("patch", "/credentials/{credential_id}"), "patchCredentialsByCredentialId")
        self.assertEqual(turbovasctl.openapi_contract_operation_id("get", "/reports/{report_id}/results"), "getReportsByReportIdResults")
        self.assertEqual(
            turbovasctl.openapi_contract_operation_id(
                "get", "/report-configs/{report_config_id}/export"
            ),
            "getReportConfigsByReportConfigIdExport",
        )
        self.assertEqual(
            turbovasctl.openapi_contract_operation_id("get", "/filters/{filter_id}/export"),
            "getFiltersByFilterIdExport",
        )
        self.assertEqual(
            turbovasctl.openapi_contract_operation_id(
                "post", "/report-configs/{report_config_id}/clone"
            ),
            "postReportConfigsByReportConfigIdClone",
        )
        self.assertEqual(
            turbovasctl.openapi_contract_operation_id(
                "post", "/report-configs/{report_config_id}/restore"
            ),
            "postReportConfigsByReportConfigIdRestore",
        )
        self.assertEqual(
            turbovasctl.openapi_contract_operation_id(
                "delete", "/report-configs/{report_config_id}/trash"
            ),
            "deleteReportConfigsByReportConfigIdTrash",
        )
        self.assertEqual(
    ...
    def test_native_tooling_state_reports_openapi_contract_drift(self):
        ...
        self.assertEqual(
            missing_exposure,
            {
                ...
                "POST /report-configs",
                "PATCH /report-configs/{report_config_id}",
                "DELETE /report-configs/{report_config_id}",
                "GET /report-configs/{report_config_id}/export",
                "DELETE /report-configs/{report_config_id}/trash",
                "POST /report-configs/{report_config_id}/clone",
                "POST /report-configs/{report_config_id}/restore",
    ...
    def test_openapi_tracks_direct_feed_security_information_and_alert_tag_lookup_contracts(self):
        ...
        expected_asset_metadata = [
            ...
            (scan_configs, "getScanConfigs", "scan-config-metadata-list-read", "scan-config-preferences-selector-import-export-create"),
            (scan_config_detail, "getScanConfigsByScanConfigId", "scan-config-metadata-detail-info-tags-and-task-backlink-read", "scan-config-preferences-selector-import-export-create"),
            (scan_config_families, "getScanConfigsByScanConfigIdFamilies", "scan-config-family-summary-read", "scan-config-preferences-selector-import-export-create"),
        ]
        ...
        self.assertEqual(alert_detail["x_turbovas_values"]["x-turbovas-inherited-still-owns"], "alert-detail-delivery-control")
        ...
        self.assertEqual(tag_resource_names["x_turbovas_values"]["x-turbovas-inherited-still-owns"], "tag-security-info-filter-actions-export")
...
"""
    ...
    def test_direct_native_api_override_is_explicit_opt_in(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn(turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV, text)
            self.assertNotIn("volumes:", text)

            file_env = turbovasctl.runtime_env(root)
            file_env.pop(turbovasctl.TURBOVAS_API_BEARER_TOKEN_ENV, None)
            file_env.pop(turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV, None)
            file_env[turbovasctl.TURBOVAS_API_DIRECT_ENV] = "1"
            command = turbovasctl.compose_command(root, "config", env=file_env)
            self.assertIn(str(override), command)
            text = override.read_text(encoding="utf-8")
            secret_path = turbovasctl.runtime_secret_path(root, turbovasctl.TURBOVAS_API_BEARER_TOKEN_SECRET)
            self.assertIn(turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV, text)
            self.assertIn(str(secret_path), text)
            self.assertIn(turbovasctl.TURBOVAS_API_BEARER_TOKEN_CONTAINER_FILE, text)
    ...
    def test_direct_native_api_runtime_env_uses_token_file_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            original_token_file = turbovasctl.os.environ.get(turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV)
            try:
                turbovasctl.os.environ.pop(turbovasctl.TURBOVAS_API_BEARER_TOKEN_ENV, None)
                turbovasctl.os.environ.pop(turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV, None)
                env = turbovasctl.native_api_direct_runtime_env(root)
            finally:
                ...
                if original_token_file is None:
                    ...
                    turbovasctl.os.environ[turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV] = original_token_file

            self.assertNotIn(turbovasctl.TURBOVAS_API_BEARER_TOKEN_ENV, env)
            self.assertEqual(env[turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV], turbovasctl.TURBOVAS_API_BEARER_TOKEN_CONTAINER_FILE)
            token = turbovasctl.native_api_direct_bearer_token(root, env)
    ...
    def test_direct_native_api_runtime_env_preserves_configured_token_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                turbovasctl.os.environ.pop(turbovasctl.TURBOVAS_API_BEARER_TOKEN_ENV, None)
                turbovasctl.os.environ[turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV] = str(token_file)
                env = turbovasctl.native_api_direct_runtime_env(root)
            finally:
                if original_token is None:
                    turbovasctl.os.environ.pop(turbovasctl.TURBOVAS_API_BEARER_TOKEN_ENV, None)
            ...
            self.assertNotIn(turbovasctl.TURBOVAS_API_BEARER_TOKEN_ENV, env)
            self.assertEqual(env[turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV], str(token_file))
            self.assertEqual(turbovasctl.native_api_direct_bearer_token(root, env), "0123456789abcdef0123456789abcdef")
    ...
    def test_direct_native_api_write_smoke_uses_guarded_operator_and_cleans_up(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report_config_clone_uuid = "55555555-5555-5555-5555-555555555556"
            ...
            def fake_run_command(command, *_args, **kwargs):
                ...
                if "INSERT INTO configs" in command_text:
                    return turbovasctl.subprocess.CompletedProcess(command, 0, scan_config_uuid + "\n", "")
                ...
                if any(part == "psql" for part in command):
                    ...
                    if "DELETE FROM targets" in command_text and "DELETE FROM targets_trash" in command_text:
                        return turbovasctl.subprocess.CompletedProcess(command, 0, "0|0\n", "")
            ...
            def fake_direct_curl(_root, path, *, method="GET", body=None, **_kwargs):
                ...
                if method == "PATCH" and path.startswith(f"/api/v1/alerts/{alert_uuid}"):
                    payload = json.loads(body)
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": alert_uuid, "comment": payload["comment"]}) + "\n200", "")
                ...
                if method == "POST" and path == "/api/v1/targets":
                    ...
                    target_live["value"] = True
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": target_uuid, "name": payload["name"], "comment": payload["comment"], "port_list": {"id": port_list_uuid, "name": "All IANA assigned TCP"}, "hosts": ["192.0.2.42"], "exclude_hosts": [], "alive_tests": payload["alive_tests"]}) + "\n201", "")
                ...
                if method == "POST" and path.startswith(f"/api/v1/targets/{target_uuid}/clone"):
                    payload = json.loads(body)
                    target_clone_live["value"] = True
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": target_clone_uuid, "name": payload["name"], "comment": payload["comment"], "hosts": ["192.0.2.42", "192.0.2.43"], "exclude_hosts": ["192.0.2.43"]}) + "\n201", "")
                if method == "POST" and path.startswith(f"/api/v1/targets/{target_uuid}/restore"):
                    target_live["value"] = True
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": target_uuid, "name": "turbovas-direct-write-smoke-target", "comment": target_updated_comment}) + "\n200", "")
                ...
                if method == "GET" and path.startswith(f"/api/v1/targets/{target_clone_uuid}"):
                    if target_clone_live["value"]:
                        return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": target_clone_uuid, "name": target_clone_name}) + "\n200", "")
                    return turbovasctl.subprocess.CompletedProcess([], 0, '{"error":{"code":"not_found"}}\n404', "")
                ...
                if method == "PATCH" and path.startswith(f"/api/v1/tasks/{task_uuid}"):
                    ...
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": task_uuid, "comment": payload["comment"]}) + "\n200", "")
                ...
                if method == "POST" and path == "/api/v1/report-configs":
                    ...
                    report_config_created_name["value"] = payload["name"]
                ...
                if method == "GET" and path.startswith(f"/api/v1/report-configs/{report_config_uuid}/export"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": report_config_uuid, "name": report_config_created_name["value"], "report_format": {"id": report_format_uuid, "name": "PDF"}}) + "\n200", "")
                if method == "PATCH" and path.startswith("/api/v1/report-configs/"):
                    payload = json.loads(body)
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": report_config_uuid, "comment": payload["comment"]}) + "\n200", "")
                if method == "POST" and path.startswith(f"/api/v1/report-configs/{report_config_uuid}/clone"):
                    payload = json.loads(body)
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": report_config_clone_uuid, "name": payload["name"], "report_format": {"id": report_format_uuid, "name": "PDF"}}) + "\n201", "")
                if method == "POST" and path.startswith(f"/api/v1/report-configs/{report_config_uuid}/restore"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": report_config_uuid, "name": report_config_created_name["value"], "report_format": {"id": report_format_uuid, "name": "PDF"}}) + "\n200", "")
                if method == "DELETE" and path.startswith(f"/api/v1/report-configs/{report_config_clone_uuid}/trash"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, "\n204", "")
                if method == "DELETE" and path.startswith("/api/v1/report-configs/"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, "\n204", "")
                ...
                if method == "POST" and path == "/api/v1/filters":
                    ...
                    if request_env.get(turbovasctl.TURBOVAS_API_DIRECT_WRITE_CONTROL_ENV) != "1":
                ...
                if method == "POST" and path.startswith(f"/api/v1/filters/{filter_uuid}/restore"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": filter_uuid, "comment": "TurboVAS direct write smoke original saved-filter comment"}) + "\n200", "")
                ...
                if method == "POST" and path.startswith(f"/api/v1/schedules/{schedule_uuid}/clone"):
                    payload = json.loads(body)
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": schedule_clone_uuid, "name": payload["name"], "comment": payload["comment"], "icalendar": "BEGIN:VCALENDAR\nVERSION:2.0\nBEGIN:VEVENT\nDTSTART:20300101T000000Z\nDURATION:PT1H\nRRULE:FREQ=DAILY\nEND:VEVENT\nEND:VCALENDAR"}) + "\n201", "")
                if method == "POST" and path.startswith(f"/api/v1/schedules/{schedule_uuid}/restore"):
                    schedule_live["value"] = True
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": schedule_uuid, "comment": ""}) + "\n200", "")
                ...
                if method == "POST" and path.startswith(f"/api/v1/scan-configs/{scan_config_uuid}/clone"):
                    payload = json.loads(body)
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": scan_config_clone_uuid, "name": payload["name"], "predefined": False}) + "\n201", "")
                ...
                if method == "POST" and path.startswith(f"/api/v1/scan-configs/{scan_config_uuid}/restore"):
                    scan_config_live["value"] = True
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": scan_config_uuid, "comment": "TurboVAS direct write smoke updated scan-config comment"}) + "\n200", "")
                if method == "GET" and path.startswith(f"/api/v1/scan-configs/{scan_config_uuid}"):
                ...
                if method == "POST" and path.startswith(f"/api/v1/port-lists/{temp_port_list_uuid}/restore"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": temp_port_list_uuid, "name": "turbovas-direct-write-smoke-port-list-1-" + str(turbovasctl.os.getpid())}) + "\n200", "")
                ...
                if method == "DELETE" and path.startswith("/api/v1/port-lists/"):
                    return turbovasctl.subprocess.CompletedProcess([], 0, '{"error":{"code":"conflict","message":"predefined port lists cannot be deleted"}}\n409', "")
                ...
                if method == "POST" and path.startswith(f"/api/v1/tags/{tag_uuid}/clone"):
                    payload = json.loads(body)
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": tag_clone_uuid, "name": payload["name"], "comment": payload["comment"], "resource_type": "cpe", "resource_count": 1}) + "\n201", "")
                if method == "POST" and path.startswith(f"/api/v1/tags/{tag_uuid}/restore"):
                    tag_deleted["value"] = False
                    return turbovasctl.subprocess.CompletedProcess([], 0, json.dumps({"id": tag_uuid, "resource_count": tag_resource_count["value"]}) + "\n200", "")
        ...
        self.assertEqual(checks["native-api-direct.report-config-metadata-export"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-create"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-update"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-clone"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-delete-original"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-post-delete-original"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-restore-original"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-delete-clone"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-post-delete-clone"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-hard-delete-clone"], "pass")
        self.assertEqual(checks["native-api-direct.report-config-write-post-hard-delete-clone"], "pass")
        ...
        self.assertEqual(checks["native-api-direct.credential-fixture"], "pass")
    ...
    def test_direct_native_api_posture_fails_broad_configured_token_file_permissions(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env[turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV] = str(token_file)
            with unittest.mock.patch.object(turbovasctl, "current_native_api_direct_published_bindings", return_value=()), unittest.mock.patch.object(turbovasctl, "running_service_env_value", return_value=None):
                findings = turbovasctl.direct_native_api_posture_findings(root, env)

        auth = {item["check"]: item for item in findings}["production.native-api-direct.auth-boundary"]
        self.assertEqual(auth["status"], "fail")
        self.assertEqual(auth["details"]["token_file_configured_value"], str(token_file))
        self.assertEqual(auth["details"]["token_file_host_path"], str(token_file))
        self.assertTrue(auth["details"]["token_file_host_metadata_available"])
        self.assertEqual(auth["details"]["runtime_secret_mode"], "0664")
        self.assertFalse(auth["details"]["runtime_secret_permission_ok"])
    ...
    def test_direct_native_api_posture_accepts_secure_configured_token_file_permissions(self):
        ...
        self.assertEqual(auth["details"]["token_sources"], ["configured-token-file"])
        self.assertEqual(auth["details"]["token_file_host_path"], str(token_file))
        self.assertEqual(auth["details"]["runtime_secret_mode"], "0600")
    ...
    def test_direct_native_api_posture_fails_for_running_non_loopback_binding(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
            turbovasctl.write_runtime_secret(
                root,
                turbovasctl.TURBOVAS_API_BEARER_TOKEN_SECRET,
                "0123456789abcdef0123456789abcdef",
            )
            def running_env_value(_root, _service, key):
                if key == turbovasctl.TURBOVAS_API_BEARER_TOKEN_FILE_ENV:
                    return turbovasctl.TURBOVAS_API_BEARER_TOKEN_CONTAINER_FILE
                return None

            with unittest.mock.patch.object(turbovasctl, "current_native_api_direct_published_bindings", return_value=running), unittest.mock.patch.object(turbovasctl, "running_service_env_value", side_effect=running_env_value):
                findings = turbovasctl.direct_native_api_posture_findings(root, {})
    ...
    def test_direct_native_api_posture_warns_for_uninspectable_running_token_file_path(self):
        running = ({"host": "127.0.0.1", "host_port": "19080", "container_port": "9081"},)
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "TurboVAS"
            root.mkdir()
        ...
        auth = {item["check"]: item for item in findings}["production.native-api-direct.auth-boundary"]
        self.assertEqual(auth["status"], "warn")
        self.assertFalse(auth["details"]["token_file_host_metadata_available"])
        self.assertIsNone(auth["details"]["token_file_host_path"])
