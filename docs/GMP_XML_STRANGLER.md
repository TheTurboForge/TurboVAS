...
saved filters, tags, overrides, port lists, schedules, report formats, and report metrics, currently raw report list/detail/result rows/hosts,
...
Scanners, Filters, Tags, Overrides, Port Lists, and Schedules lists, scope-report list, Results, Hosts, Ports, Applications,
...
| Tag metadata reads | GSA/GMP tag list/detail commands -> gsad -> gvmd XML -> PostgreSQL | `/api/v1/tags` and `/api/v1/tags/{tag_id}` | `/tags` and tag detail metadata reads now use typed JSON through the authenticated `gsad` proxy. The native payload keeps resource type/count, active state, value, and owner metadata while assigned-resource expansion and tag create, modify, clone, enable/disable, export, and delete actions remain inherited until native resource/write semantics are designed. |
...
| Read-only report/scope/target/task/config/tag/override automation | Imported GMP scripts, plus TurboVAS scope/report list scripts | `tools/turbovasctl native-api-request --json --path '/api/v1/...'` or `just native-api-request --json --path '/api/v1/...'` | Raw report, scope, scope-report, target, task, tag metadata, override metadata, report-config, and scope-report result listing now have a DB-backed native GET path. The obsolete read-only `gvm-tools` scripts are removed where equivalent native reads exist. |
