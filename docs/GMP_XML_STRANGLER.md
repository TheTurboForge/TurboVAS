...
| Runtime report/scope helpers | Some `turbovasctl` helpers still use `python-gvm`; metric helpers plus report and scope-report summary no longer use legacy XML helper paths | Native API-backed helper calls | `runtime-report-summary`, `runtime-report-metrics`, `runtime-scope-report-metrics`, and `runtime-scope-report-summary` now use the internal native API; `runtime-report-export` and other GMP probes remain temporary inherited paths until their native replacements land. |
