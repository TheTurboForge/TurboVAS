...
| Runtime report/scope helpers | Some `turbovasctl` helpers still use `python-gvm`; metric helpers no longer use the deleted legacy XML parser helper | Native API-backed helper calls | `runtime-report-metrics` and `runtime-scope-report-metrics` now use the internal native API; other GMP probes remain temporary inherited paths until their native replacements land. |
