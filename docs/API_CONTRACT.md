...
Native CVE detail also includes EPSS score/percentile plus generic URL/tag
references from `scap.cve_references`, CERT-Bund/DFN-CERT advisory references,
and NVT references that are available from PostgreSQL. EPSS provenance is not
exposed because the current `scap.epss_scores` schema stores only CVE, score,
and percentile; source/revision/import provenance would need a separate
SCAP/feed-schema design.
